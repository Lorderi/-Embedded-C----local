...
/*
	...
	int ov = 1;                  // Set peek offset to 1
    
	if(setsockopt(fd, IPPROTO_IP, IP_HDRINCL, &ov, sizeof(ov)) < 0) {
	...
	//IP header
	struct iphdr *ippacket = (struct iphdr *) datagram;
	//UDP header
	struct udphdr *udppacket = (struct udphdr *) (datagram + sizeof(struct iphdr));
	...
	ippacket->ihl = 5;
	ippacket->tos = 0;
	ippacket->tot_len = sizeof (struct iphdr) + sizeof (struct udphdr) + strlen(data);
	ippacket->id = htons (54321);	//Id of this packet
	ippacket->frag_off = 0;
	ippacket->ttl = 255;
	ippacket->protocol = IPPROTO_UDP;
	ippacket->check = 0;		//Set to 0 before calculating checksum
	ippacket->saddr = 0;	//Spoof the source ip address
	ippacket->daddr = server_address.sin_addr.s_addr;
	...
	sendto(fd, datagram, sizeof(struct udphdr) + sizeof(struct iphdr) + strlen(data), 0, (struct sockaddr *) &server_address, server_addr_size);
